...
claude -p "You are in a directory with two repos:\n 1. A mintlify docs repo: plaud-dev-docs \n2. A repo for agent skills: plaud-embedded-skills\n\nDo the following: \n1. Read the existing SKILL.md files and refrences to familiarize yourself with the current state of the Skill \n2. Verify all the referenced links in the Skill using web fetch. \n3. Verify any discrepancies between the plaud-dev-docs and the Skill. **The docs are the source of truth** \n.4. Update the Skill with any updates/changes to the docs" --allowedTools "Bash,Read,Edit" --permission-mode acceptEdits --output-format stream-json --verbose
#
